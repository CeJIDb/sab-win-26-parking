...
- [x] Фаза 11. **Прогон CI/CD на GitHub:** пользователь делает `git push` ветки `docs/portfolio-roadmap`. На GitHub Actions проходят `policy-checks`, `quality-gates` (включая `ci:check`), `commitlint`, проверка семантического заголовка PR. Это закрывающее условие — **до зеленого CI подплан 8 не считается выполненным** и подплан 1 (README) не стартует.
- [x] Фаза 12. Ретро в `docs/process/retro/2026-05-12-simplify-scripts-package-json.md`, отметка `[x]` в таблице roadmap и в фазах roadmap (`plans/2026-05-12-portfolio-roadmap.md`).
