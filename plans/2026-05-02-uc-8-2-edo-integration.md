...
- Интеграционный UC-8.2 во всем комплекте 9 deliverables.
...
Большие фазы получают отдельный подплан в `plans/`, который создается **непосредственно перед стартом фазы** после обсуждения деталей в чате. После завершения фазы подплан удаляется — итоги и ключевые решения остаются в разделе `## Итог` мастер-плана.
...
После завершения фазы ее статус в мастер-плане переключается на `[x]` со ссылкой на подплан (если он есть).

### Актуализация после merge 2026-08-25

Описания завершенных фаз ниже сохраняют историю решений на дату выполнения. Каноничная модель договора впоследствии разделена на жизненный цикл договора и состояние отдельной попытки подписания. Актуальные источники истины: [UML State Machine TO-BE](../docs/artifacts/uml-state-contract-to-be.md), [ER-модель](../docs/architecture/database/erd/erd-normalized-er-model.md) и [UC-8.2](../docs/artifacts/use-case/uc-8-2-create-contract-legal-entity.md).

Интеграционные артефакты UC-8.2 синхронизированы с этой моделью: REST-ответы используют `lifecycleStatus`, `currentSigningAttemptId`, `signingRoute` и `signingStatus`; успешное подписание завершает попытку в `SIGNED`, но не активирует договор. Исторические упоминания единого `contract_status_enum`, `AWAITING_PARKING_SIGNATURE`, `REJECTED_BY_PARKING` и активации внутри UC-8.2 считаются замененными этой актуализацией.
...
- [x] **Фаза 4. UML Sequence.** Создан [sequence-uc-8-2-create-contract.md](../docs/architecture/integration/sequence-uc-8-2-create-contract.md) с Mermaid-диаграммой, PlantUML-исходником и PNG-экспортом в [assets/](../docs/architecture/integration/assets/). Диаграмма использует 4 participants и после актуализации 2026-08-25 покрывает шаги 1–18, расширения `3а`, `8а`, `11а/11б`, `13а/13б`, inbox-дедупликацию и отдельные оси состояния. Подплан удален после завершения фазы.
- [x] **Фаза 5. JSON-пример + JSON Schema.** Созданы [JSON-примеры UC-8.2](../docs/architecture/integration/payload-uc-8-2-create-contract.md) для request и ответов `201/200/400/422/502`, а также [JSON Schema UC-8.2](../docs/architecture/integration/schema-uc-8-2-create-contract.md) draft 2020-12 с пятью `$defs`. После актуализации 2026-08-25 успешные ответы разделяют lifecycle договора и состояние текущей попытки. Подплан удален после завершения фазы.
- [ ] **Фаза 6. SOAP/WSDL-контракт для ЭДО.** Создать один WSDL-контракт передачи договора в `docs/architecture/integration/wsdl-uc-8-2-edo-contract.md` для операции `SubmitContractForSigning` (`Сервис Договоров` P14 → ЭДО E12): SOAP 1.1 `document/literal`, файл договора как `xs:base64Binary`, синхронный ответ с `идДокументаЭДО`/статусом приема, fault и inline XSD. Отдельные `.xml`, `.xsd` и `.wsdl` файлы не создавать. XML-блоки извлекать только во временный каталог для `xmllint`.
...
- [x] **Фаза 8. Swagger/OpenAPI.** Создан [OpenAPI-контракт UC-8.2](../docs/architecture/integration/openapi-uc-8-2-create-contract.md) для `POST /api/v1/contracts`: inline-схемы запроса, успешных ответов и ошибок; коды `200/201/400/401/403/422/502`. После актуализации 2026-08-25 ответ `201` фиксирует `PENDING_ACTIVATION` + `AWAITING_CLIENT_SIGNATURE`, не смешивая подписание с активацией.
...
- **Терминология «Сервис Договоров» вместо «Договор»** — в колонках источника / приемника таблицы регламента используются имена контейнеров C4 / DFD L1. Будущие архитектурные артефакты UC-8.2 (Sequence Фазы 4, OpenAPI Фазы 8, регламент Фазы 9) — пишут «Сервис Договоров». В файле требований [integration-requirements.md](../docs/specs/integration/integration-requirements.md) сохраняется логическое имя «Договор» (формат `INT-*`), это допустимое расхождение между уровнями спецификаций и архитектуры.
...
- **Две оси состояния**: после актуализации 2026-08-25 `lifecycleStatus` договора отделен от `signingStatus` текущей попытки; для UC-8.2 маршрут равен `EDO_CLIENT_THEN_OWNER`, а технические статусы ожидания и отказа владельца — `AWAITING_OWNER_SIGNATURE` и `REJECTED_BY_OWNER`.
...
- После актуализации 2026-08-25 OpenAPI, JSON Schema и ERD используют `contract_lifecycle_status_enum` и `contract_signing_status_enum`; ответ не содержит устаревшее единое поле `status`.
