...
├── scripts/                 ← скрипты сборки, линтов, atomic-commit, Codex-hooks
...
| Правила Codex (опц. ast-index) | [.Codex/rules/ast-index.md](.Codex/rules/ast-index.md)                             |
...
- `mcp__markdown_rag__index_documents` — индексация docs/. Запускай **только с явного разрешения пользователя** — операция медленная. SessionStart-хук [scripts/Codex-hooks/check-rag-index.mjs](scripts/Codex-hooks/check-rag-index.mjs) предупредит, если в `docs/` накопилось 5+ изменений после последней индексации — предложи запустить индексацию.
...
**ast-index** — опциональный бинарь для поиска по коду (JS/TS/shell/python в `scripts/`, `ui/templates/`, `.husky/`). Полные правила — [.Codex/rules/ast-index.md](.Codex/rules/ast-index.md). Если бинарь не установлен — используй Grep.

## Автоматические блокировки (Codex hooks)

В [.Codex/settings.json](.Codex/settings.json) подключены хуки в [scripts/Codex-hooks/](scripts/Codex-hooks/). Если действие заблокировано — это политика, не баг:
