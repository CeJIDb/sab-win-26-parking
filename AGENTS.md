...
├── .agents/       ← локальные skills для Codex и других агентов
├── .codex/        ← настройки Codex: config.toml, hooks.json
├── .claude/, .husky/, .github/   ← конфиги Claude Code, git-хуков и CI
...
| Настройки Codex                | [.codex/config.toml](.codex/config.toml), [.codex/hooks.json](.codex/hooks.json)   |
...
**Codex hooks** ([.codex/hooks.json](.codex/hooks.json) → [scripts/claude-hooks/](scripts/claude-hooks/)). Действие заблокировано — это политика, не баг:
