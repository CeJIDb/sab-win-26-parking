...
- Этап проекта: Этап 4. Техническое проектирование решения. Архитектура
...
Такой способ представления помогает увидеть границы ответственности. Управление клиентом и автомобилями отвечает за подготовку и актуальность данных. Контекст допуска использует эти данные для принятия решения на КПП. Контекст бронирования и договора формирует основания для допуска и использования ресурса. `Тариф` отделяется от `Расчета стоимости`: один хранит правила, другой исполняет их на конкретных параметрах сессии и брони. `Инфраструктура парковки` держит мастер-данные парковки и операционную модель мест, а отдельные интеграционные supporting-контексты, `Адаптер СКУД` и `Адаптер дисплеев и табло`, изолируют работу с оборудованием. В результате диаграмма связывает Event Storming с будущими bounded contexts, модулями, интеграционными контрактами и C4-представлением системы.

Актуальный текстовый перечень контекстов, который используем поверх рабочей доски:

- Core: `Бронирование`, `Доступ`, `Сессия`, `Тариф`, `Расчет стоимости`
...
Этот артефакт нужен перед архитектурной детализацией. С ним проще проверить, что будущие модули, таблицы, API и очереди событий не будут спроектированы "поверх картинки", а пойдут по доменным границам. Диаграмма контекстов готовит материал для DDD-артефактов и ADR и помогает связывать требования по ролям и сценариям с конкретными модулями системы.
...
- `Тариф` и `Расчет стоимости` нужно удерживать как разные Core-контексты.
...
- [C4-диаграммы цифровой платформы парковки](../c4/c4-diagrams.md) — показывают внешний контур системы (L1), контейнеры (L2) и компонентную декомпозицию (L3), внутри которой живут выделенные контексты.
