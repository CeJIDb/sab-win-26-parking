...
- [Bounded contexts](#bounded-contexts)
  ...
  - [3. Сессия](#3-сессия)
  - [4. Тариф](#4-тариф)
  - [5. Расчет стоимости](#5-расчет-стоимости)
  - [6. Платеж](#6-платеж)
  - [7. Договор](#7-договор)
  - [8. Клиент](#8-клиент)
...
С учетом [ADR-003](../adr/adr-003-modular-monolith.md) ядро домена в DDD-терминах составляют `Доступ`, `Бронирование`, `Сессия`, `Тариф` и `Расчет стоимости`. В актуализированной модели мы отдельно держим:

- `Тариф` как источник тарифных правил и версий;
- `Расчет стоимости` как контекст исполнения этих правил на конкретных входных данных;
...
Имена контекстов в DDD-документах русскоязычные, что соответствует ubiquitous language проекта. ADR-003 пока отражает более раннюю 12-модульную декомпозицию, поэтому этот документ фиксирует актуализированное уточнение границ. Полная таблица английских идентификаторов (для кода, схем БД, API-контрактов и межкомандной коммуникации):

| Английское имя модуля | Русское имя контекста                                            |
| --------------------- | ---------------------------------------------------------------- |
...
- Учитываем правило ADR-003: `Доступ` принимает решение разрешить или запретить, обращаясь к `Бронированию`, `Договору`, `Сессии`, `Платежу` и, при необходимости, к `Расчету стоимости` через задокументированные интерфейсы и `Сервис приложения`.
...
1. `Booking` — `Управление бронированием` (`Бронирование`) — Core
2. `Access` — `Управление допуском` (`Доступ`) — Core
3. `Session` — `Управление парковочной сессией` (`Сессия`) — Core
4. `Tariff` — `Управление тарифами` (`Тариф`) — Core
5. `Calculation` — `Расчет стоимости` — Core
6. `Contract` — `Управление договорами` (`Договор`) — Supporting
7. `Customer` — `Управление профилем клиента` (`Клиент` + `ТС` + `Организация`) — Supporting
8. `Employee` — `Управление профилем сотрудника` (`Сотрудник`) — Supporting
9. `Payment` — `Оплата` (`Платеж` + `Чек`) — Supporting
10. `Facility` — `Управление инфраструктурой парковки` (`Парковка` + `Сектор` + `ПМ` + `КПП`) — Supporting
11. `Support` — `Управление обращениями` (`Обращение`) — Supporting
12. `Notification` — `Управление уведомлениями` (`Уведомление`) — Generic
13. `Analytics` — `Аналитика` (`Отчет`) — Generic
14. `SkudAdapter` — `Адаптер СКУД` — Supporting
15. `DisplayAdapter` — `Адаптер дисплеев и табло` — Supporting

В документации и коде предпочтительно использовать короткие предметные имена контекстов, `Бронирование`, `Доступ`, `Сессия`, `Тариф`, `Расчет стоимости` и т. д., а управленческие заголовки использовать там, где нужно связать архитектурную модель с исходной декомпозицией контекстов.
...
| `Расчет стоимости`         | Core       | расчет стоимости, preview суммы, расчетный снимок для передачи в `Платеж`                      | каталог тарифов, проведение оплаты                                          |
...
| Ребро                                                  | Обоснование                                                                                                                     |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- |
| `Инфраструктура парковки --> Расчет стоимости`         | Расчет стоимости использует зону, тип ПМ, ограничения и конфигурацию КПП как входные параметры                                  |
| `Тариф --> Расчет стоимости`                           | Тариф поставляет версии правил и тарифные сетки, а Расчет стоимости исполняет их на конкретных данных                           |
| `Договор --> Расчет стоимости`                         | Расчет стоимости применяет договорные условия и квоты как входные ограничения или надбавки                                      |
| `Расчет стоимости --> Платеж`                          | В Платеж передается расчетный снимок и сумма, зафиксированная на момент инициации оплаты                                        |
...
> `Тариф -> Платеж` по-прежнему отсутствует. `Платеж` не должен напрямую вычислять сумму по правилам тарифа. Сначала сумма и расчетный снимок формируются в `Расчете стоимости`, затем `Платеж` фиксирует финансовый результат.
...
- `Сессия.завершить` не инициирует платеж. Сначала сумма рассчитывается в `Расчете стоимости`, затем при необходимости создается `Платеж`.
...
- `Тариф` не хранит текущую сумму к оплате и не фиксирует за собой preview. Эти функции принадлежат `Расчету стоимости`.
- `Расчет стоимости` использует правила из `Тарифа` как входные данные, а не изменяет тарифный каталог.
...
- `Расчет стоимости` использует тарифные правила из `Тарифа`, но не владеет самим каталогом тарифов.
- `Расчет стоимости` получает параметры из `Бронирования`, `Сессии`, `Инфраструктуры парковки`, `Клиента` и `Договора` как входные данные или immutable snapshots.
- `Расчет стоимости` формирует текущую сумму к оплате, preview, и расчетный снимок, который передается в `Платеж` при инициации оплаты.
- После создания `Платежа` владельцем конкретной зафиксированной суммы становится `Платеж`; `Расчет стоимости` не изменяет проведенный финансовый результат задним числом.
- Для спорных случаев `Расчет стоимости` должен быть воспроизводимым: по сохраненному расчетному снимку система может объяснить, из каких правил и параметров получилась сумма.
...
- Правила расчета суммы приходят из `Расчета стоимости`, а не определяются внутри `Платежа`.
...
- `Договор` поставляет условия в `Доступ`, `Бронирование` и `Расчет стоимости`, но не владеет фактом допуска через КПП.
- Договорные ставки и скидки передаются в `Расчет стоимости` через публичный интерфейс и `Сервис приложения`; `Тариф` при этом остается владельцем тарифного каталога.
...
- Идентификаторы клиентов и ТС используются в других контекстах как ссылки или immutable snapshots; `Сессия`, `Доступ` и `Расчет стоимости` хранят ГРЗ и профильные признаки как snapshot на момент сценария, а не как прямой FK на мастер-модель.
...
- Данные поступают через проекции событий или ETL-подобные механизмы; основные источники: `Бронирование`, `Сессия`, `Расчет стоимости`, `Платеж`, `Доступ` и другие контексты.
...
Интерфейс клиента -> Сервис приложения
  -> РасчетСтоимости.рассчитатьТекущуюСумму()  [preview]
  -> Платеж.создать(фиксированнаяСумма, источник=ЛК_КЛИЕНТА)
  -> Платеж.провести() -> Уведомление.добавитьВОчередь(чек)

// Выезд через КПП:
Адаптер СКУД -> Сервис приложения
  -> РасчетСтоимости.рассчитатьТекущуюСумму()  [если нет уже успешной оплаты]
...
| `ТС`                                                        | `Клиент`                                 | ГРЗ является атрибутом ТС; в `Сессии`, `Доступе` и `Расчете стоимости` хранится как snapshot       |
...
| `Льготный документ`                                         | `Клиент`                                 | Передается в `Расчет стоимости` как входной параметр                                               |
...
| `Расчет стоимости`, `Сумма к оплате (preview)`              | `Расчет стоимости`                       | Динамический расчет в реальном времени до момента инициации платежа                                |
| `Расчетный снимок`                                          | `Расчет стоимости`                       | Позволяет воспроизвести и объяснить, как сформировалась сумма                                      |
