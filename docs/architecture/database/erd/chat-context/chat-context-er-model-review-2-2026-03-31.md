...
> Примечание: это “заметки ревью” (исторический контекст). Они могут содержать устаревшие статусы “еще не добавлено/ожидает решения”. Источник истины по текущей модели — `docs/architecture/database/erd/erd-normalized-er-model.md` и доменные файлы `docs/architecture/database/erd/erd-relationships-*.md`.
...
Описана в ответе. Актуальный статус таблиц `REFUND` и `DEBT` (включены/не включены) фиксируется в `docs/architecture/database/erd/erd-normalized-er-model.md` и доменных файлах `docs/architecture/database/erd/erd-relationships-*.md`.
...
| ERD-007 | `CLIENT_ACCOUNT` и `EMPLOYEE_ACCOUNT` помечены схемой `auth`; FK без `REFERENCES` |
...
| ERD-027 | Новая таблица `EMPLOYEE_ACCOUNT` (схема `auth`); `login`/`password_hash`/`totp_secret_encrypted` удалены из `EMPLOYEE` |
...
- Сводная таблица ключевых связей обновлена (убрана SECTOR→BOOKING, добавлены TARIFF_RATE, EMPLOYEE_ACCOUNT, пометки о логических FK)
