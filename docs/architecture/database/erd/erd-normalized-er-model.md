...
erDiagram
    ...
    employees ||--o{ booking_status_history : records
    clients ||--o{ booking_status_history : records
...
Схема ERD из ChartDB:

![Нормализованная ER-модель из ChartDB](assets/erd-normalized-er-model.png)
...
Документ актуализируется по [DDL ERD для PostgreSQL (`public`)](../../../../sql/database/chartdb-postgresql-erd-normalized-public.sql); при расхождении приоритет отдается актуальной SQL-схеме.
...
| `contracts`                                    | **BIGINT**                                         | FK **BIGINT** на клиента; `contract_number` VARCHAR `UNIQUE`; `status` `contract_status_enum` (11 значений, включая `AWAITING_CLIENT_SIGNATURE`/`AWAITING_PARKING_SIGNATURE` для подписания через ЭДО); `client_signed_at`/`owner_signed_at` TIMESTAMPTZ                                                                                                                                      |
...
| `end_date`             | `DATE`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `status`               | `contract_status_enum` `NOT NULL` — значения: `DRAFT`, `UNDER_REVIEW`, `AWAITING_CLIENT_SIGNATURE`, `AWAITING_PARKING_SIGNATURE`, `ACTIVE`, `SUSPENDED`, `REJECTED_BY_CLIENT`, `REJECTED_BY_PARKING`, `EXPIRED`, `TERMINATED`, `ARCHIVED`. Маппинг на концептуальную модель: `DRAFT`=`черновик`, `UNDER_REVIEW`=`на согласовании`, `AWAITING_CLIENT_SIGNATURE`=`на подписании клиентом`, `AWAITING_PARKING_SIGNATURE`=`на подписании парковкой`, `ACTIVE`=`активен`, `SUSPENDED`=`приостановлен`, `REJECTED_BY_CLIENT`=`отклонен клиентом`, `REJECTED_BY_PARKING`=`отклонен парковкой`, `EXPIRED`=`просрочен` (договор не вступил в силу из-за таймаута подписи), `TERMINATED`=`расторгнут` (действующий договор прекращен после `ACTIVE`), `ARCHIVED`=`архив`. Жизненный цикл подписания через ЭДО (UC-8.2): `DRAFT` → `AWAITING_CLIENT_SIGNATURE` → `AWAITING_PARKING_SIGNATURE` → `ACTIVE`. Терминальные неуспешные ветки: `REJECTED_BY_CLIENT`, `REJECTED_BY_PARKING`, `EXPIRED` |
...
- `invoice_id` — логическая ссылка на выставленный счет (опционально; без FK-constraint);
- `start_at`;
...
- бронирование может иметь много записей истории статусов;
...
### 26а. `booking_status_history` — История статусов бронирования

Назначение: журнал изменений статуса бронирования.
...
- каждая запись относится к одному бронированию;
- каждая запись может ссылаться на одного сотрудника;
- каждая запись может ссылаться на одного клиента как инициатора изменения.
...
- одна сессия может быть логически связана с несколькими записями `access_logs`.
...
- `SINGLE` связывает счет с одной бронью через `booking_id`;
...
Назначение: просроченная задолженность клиента-ЮЛ по периодическому счету. Создается scheduled job при `invoices.due_date < current_date` и `invoices.status != 'PAID'`.
...
- `overdue_since` — дата возникновения просрочки (= `invoices.due_date`);
...
- `parking_session_id` — логическая ссылка на парковочную сессию; nullable для отказов и событий вне сессии;
...
- каждая запись может логически относиться к одной `parking_sessions`;
- каждая запись может ссылаться на ТС (логически).

Комментарий:

- таблица иммутабельна (INSERT only); обновления и удаления запрещены;
- `parking_sessions` хранит факт пребывания, а `access_logs` — детали допуска и трассировку через КПП;
...
### 38. `outbox_events` — Исходящие события

Назначение: transactional outbox для гарантированной публикации доменных событий во внешние интеграции и асинхронные обработчики.
...
- прямых FK нет;
- логическая связь с доменными агрегатами задается парой `aggregate_type + aggregate_id`.

Комментарий:

- запись создается в той же транзакции, что и доменное изменение;
- `processed_at IS NULL` означает, что событие еще не обработано или не опубликовано.
...
- `parking_sessions` 1:N `access_logs` (логическая ссылка через `access_logs.parking_session_id`)

### Интеграция и публикация событий

- `outbox_events` хранит исходящие доменные события с логической ссылкой на агрегат через `aggregate_type + aggregate_id`
...
CREATE INDEX ON invoices(due_date) WHERE status NOT IN ('PAID','CANCELLED');  -- задолженности
...
CREATE INDEX ON access_logs(parking_session_id) WHERE parking_session_id IS NOT NULL;
                                              -- корреляция AP-событий с парковочной сессией
...
2. **✅ Финансовая сумма хранится только в `INVOICE.amount_due_minor`:** связь разового счета с бронью задается через `INVOICE.booking_id`, а периодический счет живет через `contract_id` и период начисления.
