# C4 Model: Цифровая платформа парковки (SAB)

Диаграммы по модели [C4](https://c4model.com/) для системы управления парковкой. Основой служат [ADR-001](../adr/adr-001-online-access-rights-evaluation.md)–[ADR-003](../adr/adr-003-modular-monolith.md), [DDD bounded contexts](../ddd/ddd-bounded-contexts.md) и [чартер проекта](../../artifacts/project-charter.md).

## Оглавление

- [Как посмотреть диаграммы](#как-посмотреть-диаграммы)
- [Level 1 — System Context](#level-1--system-context)
- [Level 2 — Container](#level-2--container)
- [Level 3 — Component (основной процесс)](#level-3--component-основной-процесс)
- [Ключевые сценарии](#ключевые-сценарии)
- [Связанные документы](#связанные-документы)

---

## Как посмотреть диаграммы

1. **Браузер (надёжный вариант для C4):** в корне репозитория выполните `npm run docs:c4-preview` и откройте в браузере файл [`c4-parking-platform-preview.html`](c4-parking-platform-preview.html) (двойной клик по файлу или контекстное меню «Открыть с помощью…»). HTML собирается скриптом [`../../../scripts/build-c4-preview-html.mjs`](../../../scripts/build-c4-preview-html.mjs) из fenced-блоков Mermaid в этом же документе — после правок диаграмм пересоберите превью той же командой.
2. **Cursor / VS Code:** установите рекомендуемые расширения рабочей области (в т.ч. **Markdown Preview Mermaid Support**), откройте этот файл и команду **Markdown: Open Preview** (обычно Ctrl+Shift+V). Встроенный движок превью иногда отстаёт от полной сборки Mermaid: если блоки C4 пустые или с ошибкой, используйте пункт 1.
3. **GitHub:** после push fenced-блоки с языком `mermaid` в этом файле рендерятся на странице просмотра в репозитории.

---

## Level 1 — System Context

Показывает платформу как «чёрный ящик», акторов и внешние системы.

### Текстовое описание (Level 1)

**Система:** Цифровая платформа парковки (SAB) — веб-приложение для управления парковкой на 600 мест: бронирование, контроль доступа на КПП, тарификация, оплата, договоры, уведомления, аналитика.

**Акторы (люди):**

| Актор | Взаимодействие с платформой |
| --- | --- |
| Клиент ФЛ | PWA/ЛК: бронирование, оплата, история сессий, профиль, уведомления |
| Клиент ЮЛ (представитель) | ЛК: договоры, квоты, бронирование, счета, профиль организации |
| Охранник КПП | Интерфейс КПП: ручной допуск, журнал, спорные ситуации |
| Управляющий | Админ-панель: тарифы, секторы, задолженности, обращения, настройки |
| Владелец | Админ-панель: аналитика, отчёты, бизнес-решения |

**Внешние системы:**

| Система | Протокол | Назначение |
| --- | --- | --- |
| СКУД + LPR-видеосистема | UDP / REST | Распознавание ГРЗ, события въезда/выезда; исполнение allow/deny (шлагбаум) |
| Платёжный провайдер (онлайн) | REST API | Приём онлайн-оплаты (карты, СБП) |
| Платёжный терминал КПП | REST API | Оплата наличными/картой на КПП |
| ОФД | REST API | Фискализация чеков |
| SSO/IdP (Яндекс ID, VK ID) | OAuth2/OIDC | Аутентификация клиентов ФЛ |
| SMS/E-mail шлюз | REST API | Доставка уведомлений |
| ЭДО | REST API | Электронный документооборот (договоры ЮЛ) |
| Инфо-дисплеи и табло (объект) | REST API | Несколько устройств (въезд/выезд, навигация и т.д.); на L1 — один узел, см. [`c4-l1-system-context.md`](c4-l1-system-context.md) |

### Mermaid-диаграмма (Level 1)

```mermaid
C4Context
    title System Context — Цифровая платформа парковки (SAB)

    Person(client_fl, "Клиент ФЛ", "Бронирует, оплачивает,<br>следит за сессиями через PWA/ЛК")
    Person(client_ul, "Клиент ЮЛ", "Управляет договорами,<br>квотами и бронями через ЛК")
    Person(guard, "Охранник КПП", "Ручной допуск, журнал,<br>разбор спорных ситуаций")
    Person(manager, "Управляющий", "Тарифы, секторы,<br>задолженности, обращения")
    Person(owner, "Владелец", "Аналитика, отчёты,<br>бизнес-решения")

    System(platform, "Цифровая платформа парковки", "Модульный монолит (ADR-003):<br>бронирование, доступ, сессии,<br>тарификация, оплата, договоры,<br>уведомления, аналитика")

    System_Ext(skud_lpr, "СКУД + LPR", "Распознавание ГРЗ,<br>управление шлагбаумом")
    System_Ext(payment_provider, "Платёжный провайдер", "Онлайн-оплата<br>(карты, СБП)")
    System_Ext(payment_terminal, "Платёжный терминал КПП", "Оплата на КПП<br>(наличные/карта)")
    System_Ext(ofd, "ОФД", "Фискализация чеков")
    System_Ext(idp, "SSO/IdP", "Яндекс ID, VK ID —<br>аутентификация ФЛ")
    System_Ext(notify_gw, "SMS/E-mail шлюз", "Доставка уведомлений")
    System_Ext(edo, "ЭДО", "Электронный<br>документооборот")
    System_Ext(displays, "Инфо-дисплеи и табло", "Въезд/выезд,<br>навигация (неск. устр.)")

    Rel(client_fl, platform, "Бронирует, оплачивает, просматривает", "HTTPS")
    Rel(client_ul, platform, "Договоры, квоты, счета", "HTTPS")
    Rel(guard, platform, "Ручной допуск, журнал", "HTTPS")
    Rel(manager, platform, "Управление парковкой", "HTTPS")
    Rel(owner, platform, "Аналитика, отчёты", "HTTPS")

    Rel(platform, skud_lpr, "allow/deny → шлагбаум", "REST")
    Rel(skud_lpr, platform, "Событие ГРЗ", "UDP → REST")
    Rel(platform, payment_provider, "Создание/проверка платежа", "REST API")
    Rel(platform, payment_terminal, "Приём оплаты на КПП", "REST API")
    Rel(platform, ofd, "Фискализация", "REST API")
    Rel(platform, idp, "Аутентификация ФЛ", "OAuth2/OIDC")
    Rel(platform, notify_gw, "Отправка SMS/email", "REST API")
    Rel(platform, edo, "Обмен документами", "REST API")
    Rel(platform, displays, "Данные для табло", "REST API")

    UpdateLayoutConfig($c4ShapeInRow="4", $c4BoundaryInRow="1")
```

---

## Level 2 — Container

Показывает развёртываемые единицы внутри платформы: основной процесс, изолированные адаптеры, БД, фронтенды.

> Отдельные процессы выделяются только при несовместимости транспорта (UDP у СКУД/LPR) или при необходимости изолировать блокирующую асинхронную операцию (пакетная рассылка уведомлений). Платёжный терминал КПП и инфо-дисплеи/табло на объекте интегрируются напрямую через REST API из основного процесса; для каждого внешнего вендора внутри основного процесса предусмотрен ACL-слой (Anti-Corruption Layer), но как самостоятельный развёртываемый артефакт не выделяется.

### Текстовое описание (Level 2)

| Контейнер | Технология | Назначение |
| --- | --- | --- |
| **Web-приложение (PWA/ЛК)** | SPA / PWA | Клиентский интерфейс: бронирование, оплата, история, профиль |
| **Админ-панель** | SPA | Интерфейс для управляющего и владельца: тарифы, секторы, отчёты |
| **Интерфейс КПП (охранник)** | SPA | Оперативный экран охранника: текущие сессии, ручной допуск |
| **Основной процесс (API)** | Модульный монолит | Единый развёртываемый артефакт: 12 доменных модулей + Сервис приложения + инфраструктура аутентификации. Stateless. REST API. |
| **Адаптер ЛПР/СКУД** | Отдельный процесс | Принимает UDP от LPR-видеосистемы → REST-вызовы к основному процессу. Буферизация при недоступности. Anti-Corruption Layer. |
| **Агент доставки уведомлений** | Отдельный процесс | Потребляет очередь уведомлений (Outbox) → отправка через внешние шлюзы. At-least-once, idempotency key. |
| **БД PostgreSQL** | PostgreSQL | Единая БД с изоляцией по схемам (schema-per-module). Outbox-таблица для Агента доставки уведомлений. |

### Mermaid-диаграмма (Level 2)

```mermaid
C4Container
    title Container — Цифровая платформа парковки (SAB)

    Person(client_fl, "Клиент ФЛ", "")
    Person(client_ul, "Клиент ЮЛ", "")
    Person(guard, "Охранник КПП", "")
    Person(manager, "Управляющий / Владелец", "")

    System_Boundary(platform, "Цифровая платформа парковки") {
        Container(spa_client, "PWA / ЛК клиента", "SPA", "Бронирование, оплата,<br>история, профиль")
        Container(spa_admin, "Админ-панель", "SPA", "Тарифы, секторы,<br>отчёты, обращения")
        Container(spa_guard, "Интерфейс КПП", "SPA", "Активные сессии,<br>ручной допуск")
        Container(api, "Основной процесс (API)", "Модульный монолит", "12 доменных модулей,<br>Сервис приложения,<br>REST API, stateless")
        Container(lpr_adapter, "Адаптер ЛПР/СКУД", "Отдельный процесс", "UDP→REST, буферизация,<br>Anti-Corruption Layer")
        Container(notif_worker, "Агент доставки уведомлений", "Отдельный процесс", "Outbox → SMS/email/push,<br>at-least-once")
        ContainerDb(db, "PostgreSQL", "РСУБД", "Единая БД,<br>schema-per-module,<br>Outbox-таблица")
    }

    System_Ext(skud_lpr, "СКУД + LPR", "Шлагбаум, камеры ГРЗ")
    System_Ext(payment_provider, "Платёжный провайдер", "Онлайн-оплата")
    System_Ext(payment_terminal, "Платёжный терминал КПП", "Оплата на выезде")
    System_Ext(ofd, "ОФД", "Фискализация")
    System_Ext(idp, "SSO/IdP", "OAuth2/OIDC")
    System_Ext(notify_gw, "SMS/E-mail шлюз", "Доставка сообщений")
    System_Ext(edo, "ЭДО", "Электронный документооборот")
    System_Ext(displays, "Инфо-дисплеи и табло", "Въезд/выезд,<br>навигация (неск. устр.)")

    Rel(client_fl, spa_client, "Использует", "HTTPS")
    Rel(client_ul, spa_client, "Использует", "HTTPS")
    Rel(guard, spa_guard, "Использует", "HTTPS")
    Rel(manager, spa_admin, "Использует", "HTTPS")

    Rel(spa_client, api, "API-запросы", "HTTPS/JSON")
    Rel(spa_admin, api, "API-запросы", "HTTPS/JSON")
    Rel(spa_guard, api, "API-запросы", "HTTPS/JSON")

    Rel(api, db, "Читает/пишет", "SQL")
    Rel(notif_worker, db, "Читает Outbox", "SQL")
    Rel(lpr_adapter, api, "Событие ГРЗ → Доступ", "REST")

    Rel(skud_lpr, lpr_adapter, "Событие ГРЗ", "UDP")
    Rel(api, skud_lpr, "allow/deny → шлагбаум", "REST")
    Rel(api, payment_provider, "Онлайн-платежи", "REST API")
    Rel(api, payment_terminal, "Платежи на КПП", "REST API")
    Rel(api, ofd, "Фискализация", "REST API")
    Rel(api, idp, "Аутентификация", "OAuth2/OIDC")
    Rel(api, edo, "Документы", "REST API")
    Rel(api, displays, "Данные о доступности мест", "REST API")
    Rel(notif_worker, notify_gw, "Отправка", "REST API")

    UpdateLayoutConfig($c4ShapeInRow="4", $c4BoundaryInRow="1")
```

---

## Level 3 — Component (основной процесс)

Показывает доменные модули (bounded contexts) внутри основного процесса и их ключевые зависимости.

### Текстовое описание (Level 3)

Основной процесс содержит 12 доменных модулей, координируемых через Сервис приложения. Модули взаимодействуют только через публичные интерфейсы (прямой доступ к таблицам другого модуля запрещён). `Доступ` остаётся одним модулем, но внутри разделяется на `ПолитикаДопускаНаВъезд` и `ПолитикаДопускаНаВыезд`.

**Core-домены** (уникальная бизнес-логика):

| Модуль | Ответственность |
| --- | --- |
| **Доступ** | Решение разрешить/запретить на КПП; чёрный список; аудит решений. Для въезда проверяет бронь и при необходимости запрашивает авто-бронирование через интерфейс `Бронирование`; для выезда проверяет активную сессию и статус оплаты. |
| **Бронирование** | Планирование и резервирование ресурса; автоматическое бронирование по запросу `Доступ`. |
| **Сессия** | Фактическое использование парковки; журнал въезда/выезда; корректировки охранника. Всегда привязана к Бронирование (ADR-002). |
| **Тариф** | Тарифные планы, правила применимости, расчёт стоимости в реальном времени. Получает льготы и договорные ставки как входные параметры. |

**Supporting-домены:**

| Модуль | Ответственность |
| --- | --- |
| **Платёж** | Платежи, возвраты, задолженности, чеки, счета ЮЛ; фиксация зафиксированной суммы в момент оплаты и интеграция с платёжным провайдером/терминалом и ОФД. |
| **Договор** | Договоры, шаблоны, квоты, абонементные правила. |
| **Клиент** | Мастер-данные: клиент, организация, ТС (ГРЗ), ПДн, согласия. |
| **Площадка** | Инфраструктура парковки: секторы, ПМ, КПП, конфигурация и статусы. |
| **Обращение** | Обращения, тикеты, история переписки. |
| **Сотрудник** | Сотрудники, роли (RBAC), служебные профили. |

**Generic-домены:**

| Модуль | Ответственность |
| --- | --- |
| **Уведомление** | Уведомления, шаблоны, очередь на доставку. |
| **Отчёт** | Read-модели (CQRS), аналитические агрегаты, отчёты. |

**Сквозные компоненты:**

| Компонент | Ответственность |
| --- | --- |
| **Сервис приложения** | Оркестрация сценариев, пересекающих несколько модулей (въезд, выезд, оплата). |
| **Инфраструктура аутентификации** | SSO, пароли, TOTP 2FA, JWT — инфраструктурный слой, не доменный модуль. |

### Mermaid-диаграмма (Level 3)

```mermaid
C4Component
    title Component — Основной процесс (API)

    Container_Boundary(api, "Основной процесс (модульный монолит)") {

        Component(app_svc, "Сервис приложения", "Оркестрация", "Координация сквозных<br>сценариев (въезд, выезд,<br>оплата, ручной допуск)")
        Component(auth, "Инфраструктура аутентификации", "Инфраструктура", "SSO, пароли, TOTP 2FA,<br>JWT-проверка")

        Component(access, "Доступ", "Core", "разрешить/запретить на КПП,<br>политика въезда/выезда,<br>чёрный список, аудит")
        Component(booking, "Бронирование", "Core", "Бронирование,<br>авто-бронирование,<br>резервирование ПМ")
        Component(session, "Сессия", "Core", "Парковочная сессия,<br>журнал въезда/выезда")
        Component(tariff, "Тариф", "Core", "Тарифы,<br>preview суммы,<br>расчёт стоимости")

        Component(payment, "Платёж", "Supporting", "Платежи, фиксированная сумма,<br>возвраты, чеки")
        Component(contracts, "Договор", "Supporting", "Договоры, квоты,<br>абонементы")
        Component(client, "Клиент", "Supporting", "Клиент, организация,<br>ТС, ГРЗ, ПДн")
        Component(facility, "Площадка", "Supporting", "Секторы, ПМ, КПП,<br>конфигурация")
        Component(support, "Обращение", "Supporting", "Обращения, тикеты")
        Component(employee, "Сотрудник", "Supporting", "Сотрудники, RBAC")

        Component(notification, "Уведомление", "Generic", "Уведомления, шаблоны,<br>очередь (Outbox)")
        Component(report, "Отчёт", "Generic", "Read-модели, аналитика,<br>отчёты (CQRS)")
    }

    Container_Ext(lpr_adapter, "Адаптер ЛПР/СКУД", "Отдельный процесс")
    Container_Ext(spa, "SPA (клиент / админ / КПП)", "Фронтенд")
    ContainerDb_Ext(db, "PostgreSQL", "schema-per-module")

    Rel(spa, app_svc, "REST API", "HTTPS/JSON")
    Rel(lpr_adapter, app_svc, "Событие ГРЗ", "REST")

    Rel(app_svc, access, "проверитьВъезд() / проверитьВыезд()")
    Rel(app_svc, booking, "получить / завершить")
    Rel(app_svc, session, "открыть() / завершить()")
    Rel(app_svc, tariff, "рассчитатьТекущуюСумму()")
    Rel(app_svc, payment, "создать() / провести()")

    Rel(access, booking, "естьАктивнаяБронь? / создатьАвтоБронирование()")
    Rel(access, contracts, "естьАктивныйДоговор?")
    Rel(access, session, "естьАктивнаяСессия?")
    Rel(access, payment, "естьДолг? / оплатаПроведена?")
    Rel(access, client, "ГРЗ → тсИд → клиентИд")
    Rel(access, facility, "Конфигурация КПП")

    Rel(booking, facility, "Доступность ПМ")
    Rel(booking, tariff, "Предварительный расчёт")
    Rel(contracts, booking, "Квоты → бронь")
    Rel(contracts, tariff, "Договорные ставки")

    Rel(session, notification, "Событие сессии")
    Rel(booking, notification, "Событие бронирования")
    Rel(access, notification, "Отказ / предупреждение")
    Rel(payment, notification, "Чек / возврат")
    Rel(support, payment, "Запрос возврата")
    Rel(employee, access, "Ручной допуск охранника")
    Rel(employee, session, "Просмотр активных сессий")

    Rel(booking, report, "Доменные события")
    Rel(session, report, "Доменные события")
    Rel(payment, report, "Доменные события")
    Rel(access, report, "Аудит решений")

    Rel(access, db, "", "")
    Rel(booking, db, "", "")
    Rel(session, db, "", "")
    Rel(payment, db, "", "")
    Rel(client, db, "", "")
    Rel(facility, db, "", "")
    Rel(notification, db, "", "")
    Rel(report, db, "", "")

    UpdateLayoutConfig($c4ShapeInRow="4", $c4BoundaryInRow="1")
```

---

## Ключевые сценарии

### Въезд без предварительной брони

```
Камера LPR ──UDP──▶ Адаптер ЛПР/СКУД ──REST──▶ Сервис приложения
                                                       │
                                           (чтение вне транзакции):
                                           Бронирование.получитьСтатус()
                                           Договор.получитьОснование()
                                           Платёж.получитьСтатусДолга()
                                                       │
                                                       ▼
                                        ┌──────── ACID-транзакция ────────┐
                                        │  Доступ.проверитьВъезд()        │
                                        │    └─ Бронирование.создатьАвтоБронь│
                                        │       (если брони нет)          │
                                        │  Сессия.открыть(бронированиеИд) │
                                        └─────────────────────────────────┘
                                                       │
                                                       ▼
                              Сервис приложения ──REST──▶ СКУД → шлагбаум ▲
```

### Расчёт суммы, оплата и выезд

```
PWA/ЛК клиента или выездной КПП ──REST──▶ Сервис приложения
                                                       │
                                                       ▼
                                              Тариф.рассчитатьТекущуюСумму()
                                         [preview суммы в реальном времени]
                                                       │
                    ┌──────────────────────────────────┴──────────────────────────────────┐
                    ▼                                                                     ▼
      Клиент нажал «Оплатить» в ЛК                                          Автомобиль подъехал к выезду
                    │                                                                     │
                    └──────────────────────────────────┬──────────────────────────────────┘
                                                       ▼
                           Платёж.создать(фиксированнаяСумма, источник=ЛК_КЛИЕНТА | ВЫЕЗДНЫЕ_ВОРОТА)
                                                       │
                                              ┌────────┴────────┐
                                              ▼                 ▼
                                    Платёжный провайдер   Платёжный терминал КПП
                                              │                 │
                                              └────────┬────────┘
                                                       ▼
                                        ┌──────── ACID-транзакция ────────┐
                                        │  Доступ.проверитьВыезд()        │
                                        │  Бронирование.завершить()       │
                                        │  Сессия.завершить()             │
                                        │  ИсходящиеСообщения.сохранить() │
                                        └─────────────────────────────────┘
                                                       │
                                    ┌──────────────────┴──────────────────┐
                                    ▼                                     ▼
                         Уведомление.добавитьВОчередь(чек)          Отчёт.обновитьReadМодели()
                                    │
                                    ▼
                        Агент доставки уведомлений → SMS/email/push
```

---

## Связанные документы

- [ADR-001 — Онлайн-проверка доступа](../adr/adr-001-online-access-rights-evaluation.md)
- [ADR-002 — Бронирование vs Парковочная сессия](../adr/adr-002-booking-vs-session.md)
- [ADR-003 — Модульный монолит](../adr/adr-003-modular-monolith.md)
- [DDD Bounded Contexts](../ddd/ddd-bounded-contexts.md)
- [Чартер проекта](../../artifacts/project-charter.md)
- [Глоссарий проекта](../../artifacts/project-glossary.md)
- [C4 — внешний контекст и материалы](c4-external-context.md)
