...
Этот этап посвящен интеграциям и межсистемному взаимодействию. Основной объем артефактов уже собран, остаются личные доработки участников.

Закрытые направления:

- функционально-логическое проектирование интеграций — UML Sequence по UC-10.2 и UC-12.1;
- проектирование межсистемного взаимодействия — регламент взаимодействия ИС;
- интернет-технологии и форматы JSON/XML — JSON payload и JSON Schema по UC-10.2;
- интеграции через обмен сообщениями — DFD и требования для Kafka (онлайн-бронирование) и RabbitMQ (рассылка уведомлений);
- описание API-методов и интеграционных контрактов — маппинг с ЮKassa, sequence-сценарии для онлайн-оплаты и автоидентификации на въезде.

Материалы собраны в разделе `docs/architecture/integration/`.
...
В `docs/demo-days/demo-5/` собраны материалы финального демо: сценарий выступления, презентационная колода и каталог `slides/` со слайдами.

В `docs/demo-days/demo-4/` зафиксированы материалы по архитектурному блоку этапа 4 — сценарий выступления и черновики слайдов по ES TO-BE с выделенными контекстами.
...
- [Demo 4](../demo-days/demo-4/readme.md) — содержит сценарий выступления и черновики слайдов по архитектурному блоку.
