...
1. **✅ `ORGANIZATION.client_id` (1:1) зафиксировано:** `client_id BIGINT NOT NULL UNIQUE REFERENCES client(id)` — профиль ЮЛ хранится в `ORGANIZATION`, а `CLIENT` остаётся единой сущностью клиента.
