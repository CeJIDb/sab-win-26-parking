...
Смысл (упрощённая модель): профиль ФЛ хранится **в таблице `CLIENT`** (ФИО). Паспортные данные и льготные документы хранятся в схеме `pii` и связываются с `CLIENT` через `pii.*.client_id` (логические ссылки). Для ЮЛ реквизиты вынесены в `ORGANIZATION` и связаны 1:1 с `CLIENT.type='UL'`.
...
- [Нормализованная ER-модель (временный артефакт)](temp-normalized-er-model.md)
