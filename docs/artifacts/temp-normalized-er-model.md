...
erDiagram
    ...
    CLIENT_PROFILE {
        ...
        string first_name
        string middle_name
        bigint passport_data_id
        bigint benefit_document_id
    }
...
|----------|-----|----------------------------------------|
| `PARKING` | **BIGINT** | `name` VARCHAR, `address` TEXT; `parking_type_id` FK BIGINT → `PARKING_TYPE`; `operational_status_id` FK BIGINT → `OPERATIONAL_STATUS` |
...
| `PARKING_TYPE`, `OPERATIONAL_STATUS` | **BIGINT** | справочники `facility`; `code` VARCHAR UNIQUE; FK из `PARKING`, `SECTOR`, `PARKING_PLACE`, `KPP` |
| `ZONE_TYPE_VEHICLE_TYPE`, `ZONE_TYPE_TARIFF` | составной PK (BIGINT, BIGINT) | FK типов совпадают с PK `ZONE_TYPE` / `VEHICLE_TYPE` / `TARIFF` |
| `PARKING_PLACE` | **BIGINT** | `place_number` VARCHAR; FK **BIGINT** на сектор и опционально на тариф; `operational_status_id` FK BIGINT → `OPERATIONAL_STATUS` |
| `CLIENT` | **BIGINT** | `type` CHECK('FL','UL'); `status` CHECK('ACTIVE','BLOCKED','PENDING'); `organization_id` BIGINT nullable `REFERENCES organization(id)` |
| `CLIENT_PROFILE` | PK = FK **BIGINT** (`client_id`) | только для FL; `passport_data_id`/`benefit_document_id` — логические FK в схему `pii` |
| `CLIENT_ACCOUNT` | **BIGINT** | схема `auth`; `login` VARCHAR; `auth_provider` VARCHAR (открытый список); `account_status` CHECK |
| `NOTIFICATION_SETTINGS`, `PAYMENT_SETTINGS` | **BIGINT** | FK `client_id` BIGINT NOT NULL UNIQUE; булевы BOOLEAN; `monthly_limit` NUMERIC(19,4) |
...
| `PASSPORT_DATA`, `BENEFIT_DOCUMENT` | **BIGINT** | схема `pii` (152-ФЗ); `series_and_number` BYTEA (зашифровано); `document_type` CHECK; даты DATE |
| `ORGANIZATION` | **BIGINT** | ИНН/КПП/ОГРН VARCHAR; адреса TEXT; `status` CHECK('ACTIVE','BLOCKED','PENDING') |
...
| `EMPLOYEE` | **BIGINT** | `role_id` FK BIGINT → `EMPLOYEE_ROLE`; контакты VARCHAR; `status` CHECK('ACTIVE','DISMISSED','BLOCKED') |
| `EMPLOYEE_ROLE` | **BIGINT** | справочник `employee`; `code` VARCHAR UNIQUE; FK из `EMPLOYEE.role_id` |
| `EMPLOYEE_CREDENTIAL` | **BIGINT** (PK=FK) | схема `auth`; `login` VARCHAR UNIQUE; `account_status` CHECK; `totp_secret_encrypted` TEXT |
| `VEHICLE` | **BIGINT** | FK **BIGINT** на клиента и тип ТС; `license_plate` VARCHAR `UNIQUE` (с нормализацией) |
| `KPP` | **BIGINT** | FK **BIGINT** на парковку; `type` CHECK('MANUAL','AUTOMATIC','SEMI_AUTO'); `direction` CHECK; `operational_status_id` FK BIGINT → `OPERATIONAL_STATUS` |
...
| `TARIFF_RATE` | **BIGINT** | FK **BIGINT** на тариф; `rate` NUMERIC(19,4) `CHECK (rate >= 0)`; `day_of_week` SMALLINT; `time_from/to` TIME |
...
| `BOOKING` | **BIGINT** | FK логические; `status` CHECK; `type` CHECK `('AUTO','SHORT_TERM','CONTRACT')`; `start_at`/`end_at` TIMESTAMPTZ; `amount_due` NUMERIC(19,4) nullable для AUTO |
| `INVOICE` | **BIGINT** | `type` CHECK('SINGLE','PERIODIC'); `status` CHECK('ISSUED','PAID','OVERDUE','CANCELLED'); `amount_due` NUMERIC(19,4) |
| `PARKING_SESSION` | **BIGINT** | FK логические; `access_method` CHECK; `status` CHECK; `duration_minutes` GENERATED ALWAYS AS STORED |
| `PAYMENT` | **BIGINT** | FK **BIGINT** на счет; `payment_method_id` FK BIGINT → `PAYMENT_METHOD`; `status` CHECK; `amount` NUMERIC(19,4) |
| `PAYMENT_METHOD` | **BIGINT** | справочник `payment`; `code` VARCHAR UNIQUE; FK из `PAYMENT.payment_method_id` |
| `RECEIPT` | **BIGINT** | FK **BIGINT** на платеж; `fiscal_number` VARCHAR UNIQUE; `fiscal_status` CHECK('PENDING','ISSUED','FAILED'); `amount` NUMERIC(19,4) |
...
| `DEBT` | **BIGINT** | схема `payment`; FK **BIGINT** на `INVOICE`; `client_id` BIGINT логический; `amount` NUMERIC(19,4); `remaining_amount` NUMERIC(19,4) NOT NULL; `overdue_since` DATE; `status` CHECK |
...
| `ACCESS_LOG` | **BIGINT** | схема `report`; `kpp_id` BIGINT NOT NULL логический; `vehicle_id`/`client_id` BIGINT nullable; `direction` CHECK('IN','OUT'); `decision` CHECK('ALLOW','DENY','MANUAL'); `decided_at` TIMESTAMPTZ NOT NULL; append-only |
...
> **Схема `facility`.** Справочник типов парковочных объектов. Используется в `PARKING.parking_type_id`.
...
Примеры значений `code`: `SURFACE`, `MULTILEVEL`, `UNDERGROUND`, `ROOFTOP`. *DrawSQL: добавить через Table Notes.*

### `OPERATIONAL_STATUS`

> **Схема `facility`.** Единый справочник эксплуатационных статусов для инфраструктурных объектов: `PARKING`, `SECTOR`, `PARKING_PLACE`, `KPP`. Общая таблица гарантирует согласованность набора значений.
...
| `override_tariff_id` | `BIGINT` `REFERENCES tariff(id)` |
...
| `status_reason` | `TEXT` |
| `organization_id` | `BIGINT` `REFERENCES organization(id)` — `NOT NULL` при `type='UL'`, `NULL` при `type='FL'`. `CLIENT` и `ORGANIZATION` находятся в одной схеме `client`, поэтому FK-constraint безопасен. Инвариант (NOT NULL для UL) проверяется триггером или Application Service. |
...
| `passport_data_id` | `BIGINT` — логическая ссылка на `pii.passport_data(id)` (без `REFERENCES`; схемная изоляция) |
| `benefit_document_id` | `BIGINT` — логическая ссылка на `pii.benefit_document(id)` (без `REFERENCES`; схемная изоляция) |
...
`inn NOT NULL UNIQUE` — ИНН обязателен (Q6); устраняет потенциальное нарушение BCNF: ИНН функционально определяет организацию, без UNIQUE `name`/`kpp` транзитивно зависели бы от `inn`, а не от `id`.
...
| `type` | `VARCHAR(32)` `NOT NULL` `CHECK (type IN ('AUTO', 'SHORT_TERM', 'CONTRACT'))` — `AUTO`: создаётся системой при въезде ТС через КПП; `SHORT_TERM`: краткосрочное бронирование клиентом; `CONTRACT`: долгосрочное по договору (ЮЛ) |
| `status` | `VARCHAR(32)` `NOT NULL` `CHECK (status IN ('PENDING','CONFIRMED','ACTIVE','COMPLETED','CANCELLED','NO_SHOW'))` |
| `amount_due` | `NUMERIC(19, 4)` — NULL при создании AUTO-брони (сумма не известна на момент въезда); заполняется Application Service при завершении сессии. NOT NULL для SHORT_TERM и CONTRACT. Инвариант проверяется Application Service |
...
| `amount` | `NUMERIC(19, 4)` `NOT NULL` — сумма задолженности на момент создания; иммутабельна |
| `remaining_amount` | `NUMERIC(19, 4)` `NOT NULL` — текущий остаток долга; инициализируется `= amount`; уменьшается Payment Service атомарно при каждой частичной оплате; `CHECK (remaining_amount >= 0 AND remaining_amount <= amount)` |
...
`vehicle_id` и `client_id` — nullable: при некоторых отказах (нераспознанный ГРЗ) идентифицировать ТС или клиента невозможно. Таблица immutable (INSERT only). *DrawSQL: CHECK не поддерживается в UI — указать в Table Notes.*
...
### 5а. `PARKING_TYPE` — Тип парковочного объекта

Назначение: справочник типов парковочных объектов. Заменяет поле `PARKING.type VARCHAR` — позволяет расширять список типов без изменения схемы.

Ключевые поля:

- `id`;
- `code` — строковый код (например, `SURFACE`, `MULTILEVEL`, `UNDERGROUND`, `ROOFTOP`);
- `name` — отображаемое наименование;
- `description` — описание.

Связи:

- один тип парковки назначается многим объектам `PARKING`.

### 5б. `OPERATIONAL_STATUS` — Эксплуатационный статус

Назначение: единый справочник эксплуатационных статусов для инфраструктурных объектов. Используется таблицами `PARKING`, `SECTOR`, `PARKING_PLACE`, `KPP` через поле `operational_status_id`.
...
- один статус назначается многим объектам через `*_id`-поля в `PARKING`, `SECTOR`, `PARKING_PLACE`, `KPP`.

Комментарий:

- единый справочник вместо четырёх отдельных `CHECK`-ограничений исключает расхождение набора допустимых значений между таблицами инфраструктуры.
...
- `amount_due` — снимок расчёта тарифа: NULL при создании AUTO-брони (сумма неизвестна на въезде); заполняется Application Service при завершении сессии; NOT NULL для SHORT_TERM и CONTRACT; не пересчитывается автоматически; юридически авторитетны суммы в `INVOICE.amount_due`;
...
Назначение: append-only журнал каждого события допуска (въезд или выезд) через КПП. Принадлежит схеме `report`. Создаётся системой при каждом решении о допуске — автоматическом или ручном.

Ключевые поля:

- `id`;
- `kpp_id` — логическая ссылка на КПП (без FK-constraint);
...
- используется для аудита, отчётности и расследования инцидентов;
- `direction` — явное поле (не выводится из KPP.direction), т.к. КПП может быть двусторонним.
...
- `KPP` 1:N `ACCESS_LOG` (append-only, логические FK)
...
CREATE INDEX ON kpp(parking_id);              -- все КПП парковки
CREATE INDEX ON parking_place(sector_id);     -- все места сектора

-- ═══════════════════════════════════════════════════════
-- ПАРКОВОЧНЫЕ СЕССИИ — КПП-ссылки
-- ═══════════════════════════════════════════════════════
CREATE INDEX ON parking_session(entry_kpp_id);   -- события на въезде КПП
CREATE INDEX ON parking_session(exit_kpp_id);    -- события на выезде КПП
...
-- ═══════════════════════════════════════════════════════
-- ACCESS_LOG (append-only; основные пути отчётов)
-- ═══════════════════════════════════════════════════════
CREATE INDEX ON access_log(kpp_id);                                          -- события по КПП
...
| `BOOKING.amount_due` | **3НФ — намеренная денормализация** | Снимок расчёта тарифа на момент создания брони; не пересчитывается автоматически. NULL для AUTO-броней до завершения сессии |
...
| `ORGANIZATION.inn` | **BCNF ✅ исправлено** | `inn VARCHAR(12) NOT NULL UNIQUE`; `ogrn UNIQUE`. ИНН обязателен (Q6) |
