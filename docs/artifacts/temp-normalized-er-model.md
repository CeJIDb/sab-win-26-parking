...
| `TARIFF_RATE` | **BIGINT** | FK **BIGINT** на тариф; `rate` NUMERIC(19,4); `day_of_week` SMALLINT; `time_from/to` TIME |
...
| `BOOKING` | **BIGINT** | FK логические; `status` CHECK; `type` CHECK `('AUTO','SHORT_TERM','CONTRACT')`; `start_at`/`end_at` TIMESTAMPTZ; `amount_due` NUMERIC(19,4) |
...
| `RECEIPT` | **BIGINT** | FK **BIGINT** на платеж; `fiscal_status` CHECK('PENDING','ISSUED','FAILED'); `amount` NUMERIC(19,4) |
...
| `DEBT` | **BIGINT** | схема `payment`; FK **BIGINT** на `INVOICE`; `client_id` BIGINT логический; `amount` NUMERIC(19,4); `overdue_since` DATE; `status` CHECK |
