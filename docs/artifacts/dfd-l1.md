...
P14 (Contract Service) формирует договор по входящим данным и шаблонам, при необходимости читает из БД профиль клиента (`Профиль клиента`), пишет договор, попытки подписания, входящие события и две истории состояний в БД (`Договор, событие`) и отправляет договор на подпись контрагенту через ЭДО (E12: «Договор на подпись» / «Подписанный договор»). События ЭДО меняют статус текущей попытки подписания, но не присваивают договору статус «Активен». P14 является единственным владельцем атрибута `статусЖизненногоЦикла` и выполняет `EvaluateContractActivation` после независящего от транспорта подтверждения полной оплаты от P15 (S10), события подписания, наступления даты начала или снятия блокировки. P2 (Billing Service) выступает единой точкой интеграции с 1С (E13: «Платеж, возврат, долг»): агрегирует данные оплат, парковочных сессий, клиентов и финансовых данных договоров, формирует одну сводную выгрузку в 1С и принимает мастер-данные (справочники, реквизиты, акты сверки) одной стрелкой. Это сохраняет единую точку интеграции и согласуется с C4 L3.
...
| D10 | P4 Access Control   | ТС, бронирование, профиль клиента, платеж, долг, договор | R: политика доступа через `v_access_*`; договор только со статусом «Активен» без архивной отметки (ADR-005) |
...
| D10 DB → P4 (ТС, бронирование, профиль клиента, платеж, долг, договор)   | Прямое read-only чтение чужих агрегатов через `v_access_*` для горячего пути allow/deny; договор участвует только при статусе жизненного цикла «Активен» без архивной отметки — [ADR-005](../architecture/adr/adr-005-access-control-direct-db-read.md) |
...
- Контроль доступа P4 показан как агрегатор политики: он читает чужие агрегаты (бронь, сессия, ТС, статус, долг, договор) одной стрелкой `DB → P4` (D10) через view (ADR-005) и пишет аудит решения в свой агрегат `access_logs` (D11). Договорное view возвращает только договоры со статусом «Активен» без архивной отметки. Pricing на выезде P4 не дергает напрямую: для краткосрочного бронирования (UC-12.6 ветка 3а) расчет требуемой суммы делегирован владельцу активной ПС — P5 синхронно вызывает P16 (S08) и через S05/A03 закрывает шлагбаум.
...
- Автоуведомления и обновления жизненного цикла бронирования реализованы через transactional outbox: P1, P15, P5, P14, P2 пишут событие в `outbox_events` в одной транзакции с записью своего агрегата ([ADR-003](../architecture/adr/adr-003-modular-monolith.md) инв. 4); потребители — P9 (уведомления) и P1 (жизненный цикл бронирования). Запуск проверки активации по полной оплате показан отдельно как S10 без привязки к транспорту; конкретный транспорт — деталь L2.
