...
> Примечание: это “заметки ревью” (исторический контекст). Они могут содержать устаревшие статусы “еще не добавлено/ожидает решения”. Источник истины по текущей модели — `docs/artifacts/erd/erd-normalized-er-model.md` и доменные файлы `docs/artifacts/erd/erd-relationships-*.md`.
...
Описана в ответе. Актуальный статус таблиц `REFUND` и `DEBT` (включены/не включены) фиксируется в `docs/artifacts/erd/erd-normalized-er-model.md` и доменных файлах `docs/artifacts/erd/erd-relationships-*.md`.
